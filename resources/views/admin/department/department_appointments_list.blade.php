                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-outline card-primary p-3">
                        <form action="{{route('generate_csv_submitted')}}" method="POST" id="contact_form">
                            @csrf()
                            <input type="hidden" name="csv_type" value="appointments">
                            <div class="optionFilter">
                                <div class="row">

                                    <div class="col-md-7">
                                        <i class="fa fa-cubes"></i> {{$depData->name}}
                                    </div>

                                    <div class="col-md-5" style="display: flex;">
                                        <a href="{{ route('department_list') }}" class="btn btn-default">
                            ...
                            <table id="example1" class="table table-head-fixed text-nowrap">
                                ...
                                <tbody>
                                    ...
                                    <tr id="TR__{{$rowData->id }}">
                                        ...
                                        <td>
                                            <a href="{{route('appointment_details', $rowData->id)}}">{{ $rowData->ref_id }}</a>
                                        </td>
                                        <td>{{ ($rowData->department_name == '') ? 'Office Shutters UK' : $rowData->department_name }}</td>
                                        <td>{{ $rowData->ap_title." ".$rowData->ap_firstname." ".$rowData->ap_surname  }}</td>
                                        <td>{{ $rowData->ap_postcode }}</td>

                                        <td>
                                            <input type="checkbox" name="selectIds[]" class="checkbox" value="{{$rowData->id }}" id="checkIds_{{$rowData->id }}">
                                        </td>
                                    </tr>
                                    @endforeach
                                    @endif
