            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="card card-outline card-primary p-3">
                        <form action="{{ route('updateOrderStatus_multiple') }}" method="post" id="order_list_form">
                            @csrf()
                            <div class="optionFilter">
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        <div class="row">
                                            <div class="col-8">
                                                <select name="status_top" class="form-control">
                                            ...
                                            <div class="col-4">
                                                <button type="submit" class="btn btn-info btn-sm mt-1 w-100" disabled id="orderComp">Go</button>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                        <input type="hidden" name="generate_pdf" id="generate_pdf_input">
                                        <button class="generate_pdf btn btn-warning" style="margin: auto;display: block;margin-top: 2px;" disabled>
                                        ...
                                        <tr id="TR__{{$order->id}}">
                                            ...
                                            <td>
                                                <a href="{{ route('user_details', $order->user_id) }}">
                                                    {{ $order->b_title." ".$order->b_firstname ." ".$order->b_lastname
                                                    }}
                                                </a>
                                            </td>
                                            <td>£{{ $order->total_price }}</td>
                                            <td><input type="checkbox" class="checkbox_order" name="checkbox_order[{{$order->id}}]" value="{{$order->id}}"></td>
                                            ...
                                            <td>
                                                <div class="btn-group">
                                                    ...
                                                    <div class="dropdown-menu" role="menu">
                                                        @if($status != 'Cancelled')
                                                        <a class="dropdown-item" href="{{ route('updateOrderCancel', $order->id) }}" onclick="return confirm('Are you sure you want to cancel order #<?= $order->id ?>?')">
                                                            Cancel
                                                        </a>
                                                        @endif

                                                        <a class="dropdown-item" href="{{ route('orderDeleteAndRestore', $order->id) }}" onclick="return confirm('Are you sure you want to delete order #<?= $order->id ?>?')">
                                                            Delete
                                                        </a>
                            ...
                            <div class="mt-2">
                                <!-- <button class="generate_pdf btn btn-warning" type="button">
                                    <i class="fa fa-file-download"></i>
                                    Download PDF
                                </button> -->
    ...
    $(document).ready(function() {
        ...
        $('#selectAllCheck').click(function() {
            if ($(this).prop('checked') == true) {
                ...
                $("#orderComp, .generate_pdf").attr('disabled', false);
            } else {
                $(".checkbox_order").prop('checked', false);
                $("#orderComp, .generate_pdf").attr('disabled', true);
            }
        });
